...
public class IndexBar extends View {
    ...
    private List<ContactBean> mSourceDatas;//Adapter的数据源
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPaint.setColor(Color.parseColor("#006eff"));
    ...
    public IndexBar setmSourceDatas(List<ContactBean> mSourceDatas) {
    ...
    private void initSourceDatas() {
        ...
        for (int i = 0; i < size; i++) {
            ContactBean bean = mSourceDatas.get(i);
            if (!mIndexDatas.contains(bean.getFirstHeadLetter())) {
                mIndexDatas.add(bean.getFirstHeadLetter());
            }
        }
//        sortData();
    }
    ...
    private void sortData() {
        ...
        //对数据源进行排序
        Collections.sort(mSourceDatas, new Comparator<ContactBean>() {
            @Override
            public int compare(ContactBean lhs, ContactBean rhs) {
                if (lhs.getFirstHeadLetter().equals("#")) {
                    return 1;
                } else if (rhs.getFirstHeadLetter().equals("#")) {
                    return -1;
                } else {
                    return lhs.getFirstHeadLetter().compareTo(rhs.getFirstHeadLetter());
                }
            }
        });
    }
    ...
    private int getPosByTag(String tag) {
        ...
        for (int i = 0; i < mSourceDatas.size(); i++) {
            if (tag.equals(mSourceDatas.get(i).getFirstHeadLetter())) {
                return i;
            }
        }
        return -1;
